...
class BiologyQuestionEvaluator:
    ...
    def process_questions(self) -> List[Dict]:
        ...
        for _, row in df.iterrows():
            ...
            question_data['llm_prompt'] = self.generate_llm_prompt(question_data)
    ...
    def generate_llm_prompt(self, question_data: Dict) -> str:
        ...
        """
        base_prompt = (
            "You are answering multiple choice questions about biology using information from a textbook.\n"
            "Use ONLY the context provided to determine your answer. If the context doesn't contain enough information, choose the best answer based on the provided context.\n\n"
            f"{question_data['prompt_content']['question']}\n\n"
            f"A) {question_data['prompt_content']['options']['A']}\n"
            f"B) {question_data['prompt_content']['options']['B']}\n"
            f"C) {question_data['prompt_content']['options']['C']}\n"
            f"D) {question_data['prompt_content']['options']['D']}\n\n"
    ...
    def extract_answer(self,response):
        """Extract answer from response, looking only for JSON format."""
        try:
            # Find the last occurrence of a JSON-like structure
            last_brace_start = response.rfind('{')
            last_brace_end = response.rfind('}')
            ...
            if last_brace_start != -1 and last_brace_end != -1:
                ...
                # Validate the answer
                if ('answer' in answer_json and 
                    isinstance(answer_json['answer'], str) and 
                    answer_json['answer'].upper() in {'A', 'B', 'C', 'D'}):
                    return answer_json['answer'].upper()
