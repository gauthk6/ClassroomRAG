...
class EvaluationPipeline:
    def __init__(self, 
        ...
        self.setup_rag(force_refresh=True)  # Force refresh to ensure vector store is populated
    ...
    def setup_rag(self, force_refresh: bool = False):
        ...
        if not self.rag_system.sections:
            print("Extracting sections from PDF...")
            self.rag_system.extract_sections()
        
        # Create or load vector store
        print("Setting up vector store...")
        self.vectorstore = self.rag_system.create_vectorstore(force_refresh=force_refresh)
        print(f"Vector store has {self.vectorstore._collection.count()} documents")
    ...
    def evaluate_single_question(self, question_data: Dict) -> Dict:
        ...
        try:
            ...
            retrieved_context = "\n\n---\n\n".join(
                ...
                for text in rag_result['source_texts']
            )
            ...
            return {
                ...
                'error_message': str(e),
                'prompt': prompt,
                'llm_response': rag_result if 'rag_result' in locals() else None,
                'raw_response': rag_result['answer'] if 'rag_result' in locals() else None,
                'timestamp': datetime.now().isoformat()
            }
    ...
    def run_evaluation(self, 
        ...
        for question in tqdm(questions, desc="Processing questions"):
            ...
            metrics['source_sections_used'].update(
                result['llm_response']['source_sections']  
            )
...
def main():
    ...
    test_results = pipeline.run_evaluation(max_questions=30)
